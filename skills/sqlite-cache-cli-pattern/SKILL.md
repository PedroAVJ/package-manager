...
description: Design and implement a durable plugin-owned CLI that syncs API, export, or connector data into a local SQLite cache for reliable offline querying. Use when an owning plugin needs repeatable search, normalized identity resolution, rosters, incremental sync, import/export support, or local joins that a connector cannot provide directly.
---

# SQLite Cache CLI Pattern

Use this implementation pattern inside the plugin that owns the application or
service. Do not create a loose machine-wide CLI merely because the pattern is
reusable. The CLI should turn a source that is good for live reads but bad for
complete querying into a local, inspectable database with explicit sync/import
steps and fast query commands.
...
Example: a raw Gmail CLI can be useful without SQLite because the source API
already resolves exact messages and threads. Add a cache only when it enables a
real missing capability such as offline cross-thread analysis, durable local
joins, or repeatable history that the source cannot query directly.
